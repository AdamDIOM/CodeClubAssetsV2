...
function identifyChanges(data, originalData) {
    changes = []
    if(data.LengthKept != originalData.LengthKept) changes.push({"LengthKept" : originalData.LengthKept})
...
app.http('updateTrackedKit', {
    methods: ['PUT'],
    authLevel: 'user',
    handler: async (request, context) => {
        context.log(`updateTrackedKit called at "${request.url}"`);
        ...
        try {
            ...
            const result = await pool.request()
                ...
                .input('LengthKept', sql.Int, assetData.LengthKept)
                .query(`
                    UPDATE assets.KitTracking
                    SET
                    LengthKept = @LengthKept
                    WHERE ID = @ID;
                `);
            ...
            return { 
                status: 200,
                body: JSON.stringify(`Tracked Kit Instance ${assetData.ID} updated successfully.`)
            };
        } catch (err) {
