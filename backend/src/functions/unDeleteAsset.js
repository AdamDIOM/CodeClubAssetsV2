...
app.http('unDeleteAsset', {
    route: 'unDeleteAsset/{id}',
    methods: ['PUT'],
    authLevel: 'user',
    handler: async (request, context) => {
        const id = request.params.id;
        context.log(`unDeleteAsset called at "${request.url}"`);
        ...
        try {
            ...
            return { 
                status: 200,
                body: JSON.stringify(`Asset ${id} un-deleted successfully.`)
            };
        } catch (err) {
            ...
            return {
                status: 500,
                body: JSON.stringify("Failed to un-delete asset.")
            }
        }
    }
});
